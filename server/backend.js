...
app.post('/register/signup', async function (req, res) {
    let conn;
    try {
        const { collegeid, firstname, lastname, email, contact, password } = req.body;
        if (!collegeid || !firstname || !lastname || !email || !contact || !password) {
        ...
        const result = await collection.insertOne({ collegeid, firstname, lastname, email, contact, password });
        await conn.close();
...
app.post('/adminregister/signup', async function (req, res) {
    ...
    try {
        ...
        const result = await collection.insertOne({ idno, firstname, lastname, email, contact, password });
        await conn.close();
...
app.post('/user/forgot-password', async function (req, res) {
    ...
    try {
        ...
        res.status(500).json({ error: "Failed to validate email", details: err.message });
    }
});
...
app.delete('/api/events/:id', async (req, res) => {
    ...
    try {
        ...
        const result = await db.collection("events").deleteOne({ _id: new ObjectId(id) }); // Use ObjectId
...
app.post('/api/events/:eventId/register', async (req, res) => {
    ...
    try {
        ...
        const updatedParticipants = event.participants ? [...event.participants, collegeid] : [collegeid];
        await db.collection("events").updateOne(
            { _id: new ObjectId(eventId) }, 
            { $set: { participants: updatedParticipants } }
        );

        res.json({ message: 'Successfully registered for the event', title: event.title });
    } catch (error) {
...
app.post('/api/events/:eventId/attendance', async (req, res) => {
    ...
    try {
        ...
        if (existingEvent) {
            ...
            const alreadyMarkedIds = presentCollegeIds.filter(id => existingEvent.collegeIds.includes(id));
            if (alreadyMarkedIds.length > 0) {
                return res.status(400).json({ message: `Attendance already submitted for participants with college IDs: ${alreadyMarkedIds.join(', ')}` });
            }

            // Update the event document by adding new college IDs to the array
            await db.collection("eventsattendance").updateOne(
                { eventId: new ObjectId(eventId) },
